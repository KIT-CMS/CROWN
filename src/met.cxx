...
/**
 * @brief Applies Type-1 MET corrections by propagating jet energy corrections
 * to the MET vector.
 *
 ...
 * @param jet_muon_subtr_delta_phi name of the column with the $\phi$ difference
 * between before muon subtraction and after muon subtraction for jets
 * @param jet_ch_em_ef name of the column with jet charged electromagnetic
 * fraction
 * @param jet_ne_em_ef name of the column with jet neutral electromagnetic
 * fraction
 * @param low_pt_jet_phi name of the column with corrected jet $\phi$'s
 * @param low_pt_jet_muon_subtr_delta_phi name of the column with the $\phi$
 * difference between before muon subtraction and after muon subtraction for
 * low-pt jets
 * @param low_pt_jet_em_ef name of the column with low-pt jet electromagnetic
 * fraction
 *
...
Type1Correction(ROOT::RDF::RNode df,
    ...
    auto correction_lambda = [](const ROOT::Math::PtEtaPhiMVector &raw_met,
        ...
        ROOT::RVec<float> phis(jet_corr_pts.size());
        phis = ROOT::VecOps::Concatenate(jet_phis, low_pt_jet_phis);
...
TypeIMET(ROOT::RDF::RNode df,
    ...
    auto jet_energy_scale_sf =
        [jes_l1_evaluator, jes_l2_evaluator, jes_l2l3_evaluator, jes_evaluator]
    ...
    auto correction_lambda = [jes_l1_evaluator,
        ...
        for (std::size_t i = 0; i < pts.size(); ++i) {
            ...
            if (!is_data && gen_pts.size() > 0) {
                if (jes_shift != 0.0) {
                    ...
                    if (jes_shift_sources.at(0) != "HEMIssue") {
                        if (jet_energy_scale_shifts.size() == 1) {
                            ...
                            jesShift = 1. + jes_shift * std::sqrt(quad_sum);
                            Logger::get("physicsobject::jet::PtCorrectionMC")
                ...
                // --- JER (MC only) ---
                float reso = jet_energy_resolution(etas.at(i), ptCorr, rho);
                float sJer = 1.0;
                if (std::stoi(era.substr(0, 4)) <= 2018) { // run 2 case
                    sJer = jer_sf_evaluator->evaluate({etas.at(i), jer_shift});
                } else { // run 3 case
                    sJer = jer_sf_evaluator->evaluate({etas.at(i), ptCorr, jer_shift});
                }

                bool matched = false;
                double genPtForSmear = -1.0;
                // gen jet matching algorithm for JER
                ROOT::Math::RhoEtaPhiVectorF jet(ptCorr, etas.at(i), phis.at(i));
                ...
                for (int j = 0; j < gen_pts.size(); j++) {
                    ...
                    if (delta_r > min_delta_r)
                        continue;
                    if (delta_r < (jet_radius / 2.) && std::abs(ptCorr - gen_pts.at(j)) <
                            (3.0 * reso * ptCorr)) {
                        min_delta_r = delta_r;
                ...
                if (gen_jet_pt > 0.0) {
                    ...
                    jerShift = (sJer - 1.0) * (ptCorr - gen_jet_pt) / ptCorr;
                } else {
                    Logger::get("physicsobject::jet::PtCorrectionMC")
                        ->debug("No gen jet found. Applying stochastic smearing.");
                    
                    if (2.5 < abs(etas.at(i)) || abs(etas.at(i)) > 3) {
                        jerShift = randm.Gaus(0, reso) *
                                std::sqrt(std::max(sJer * sJer - 1., 0.0));
                    }
                }
            ...
            // --- Type-1 jet selection ---
            // only propagate objects above the given pt threshold
            if (ptCorr > 15.0 && abs(etas.at(i)) < 5.2 && (EmEf.at(i)) < 0.9) {
                // Apply (full − L1) to MET
        ...
        corrected_met.SetPxPyPzE(MetX, MetY, 0,
                                    std::sqrt(MetX * MetX + MetY * MetY));
        Logger::get("physicsobject::TypeIMET")->debug("old met {}", raw_met.Pt());
        Logger::get("physicsobject::TypeIMET")
