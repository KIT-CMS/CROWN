...
Type1Correction(
    ...
    auto type1_corr_func = [
        ...
        auto t1jet_p3_corrected = ROOT::VecOps::Construct<ROOT::Math::RhoEtaPhiVector>(
            t1jet_pt_corrected[mask], t1jet_eta[mask], t1jet_phi[mask]
        );
        auto t1jet_p3_l1corrected = ROOT::VecOps::Construct<ROOT::Math::RhoEtaPhiVector>(
            t1jet_pt_l1corrected[mask], t1jet_eta[mask], t1jet_phi[mask]
        );
        auto pt_vec_diff = ROOT::VecOps::Sum(
            t1jet_p3_corrected - t1jet_p3_l1corrected,
            ROOT::Math::RhoEtaPhiVector(0.0, 0.0, 0.0)
        );

        // Calculate the corrected MET vector
        auto met_corrected_3d = ROOT::Math::RhoEtaPhiVector(
            raw_met.Pt(),
            0.0,
            raw_met.Phi()
        ) + pt_vec_diff;
