...
ROOT::RDF::RNode Build(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(
        outputname,
        [position](const ROOT::RVec<float> &pts, const ROOT::RVec<float> &etas,
            ...
            try {
                ...
                Logger::get("lorentzvector::Build")
                    ->debug("Used object {} with quantities pt {}, eta {}, phi "
                            "{}, mass {}",
                            index, pts.at(index), etas.at(index),
                            phis.at(index), masses.at(index));
            ...
            Logger::get("lorentzvector::Build")->debug("P4 : {}", p4);
            return p4;
        },
 ...
 * @param mass name of the column containing the mass values of an object
 * for the Lorentz vector
 * @param index index of an object that specifies for which object in the object
 * vector the Lorentz vector should be build
 *
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode Build(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(
        outputname,
        [index](const ROOT::RVec<float> &pts, const ROOT::RVec<float> &etas,
                const ROOT::RVec<float> &phis,
                const ROOT::RVec<float> &masses) {
            ROOT::Math::PtEtaPhiMVector p4;
            try {
 ...
 * kinematic values for \f$p_T\f$, \f$\eta\f$, \f$\phi\f$ and mass. This
 * function utilizes the [PtEtaPhiMVector from
 * ROOT](https://root.cern/doc/master/namespaceROOT_1_1Math.html#a6cea5921731c7ac99dea921fb188df31).
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the Lorentz vector
 * @param pt name of the column containing the \f$p_T\f$ value for the Lorentz
 * vector
 * @param eta name of the column containing the \f$\eta\f$ value for the Lorentz
 * vector
 * @param phi name of the column containing the \f$\phi\f$ value for the Lorentz
 * vector
 * @param mass name of the column containing the mass value for the Lorentz
 * vector
 *
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode Build(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(
        ...
        [](const float &pt, const float &eta, const float &phi,
           const float &mass) {
            Logger::get("lorentzvector::Build")
                ->debug("Used object quantities pt {}, eta {}, phi {}, mass {}",
                        pt, eta, phi, mass);
            ROOT::Math::PtEtaPhiMVector p4;
            if (pt >= 0.) {
 ...
 * momentum/energy (MET) with given kinematic values for \f$p_T\f$ and
 * \f$\phi\f$. For MET, \f$\eta\f$ and the mass are assumed to be zero. This
 * function utilizes the [PtEtaPhiMVector from
 * ROOT](https://root.cern/doc/master/namespaceROOT_1_1Math.html#a6cea5921731c7ac99dea921fb188df31).
 *
 * @param df input dataframe
 ...
 * @param met_pt name of the column containing the \f$p_T\f$ value for the
 * Lorentz vector
 * @param met_phi name of the column containing the \f$\phi\f$ value for the
 * Lorentz vector
 *
 * @return a new dataframe containing the new column
 */
...
/**
 * @brief Build a new column with a collection of Lorentz vectors per dataframe
 * entry, which are created from the \f$p_T\f$, \f$\eta\f$, \f$\phi\f$ and mass
 * columns of a collection (e.g. jets or muons).
 *
 * For instance, this can be used to create four-vector objects from the
 * four-vector component columns of a NanoAOD collection.
 *
 * The function expects \f$p_T\f$, \f$\eta\f$, \f$\phi\f$ and mass columns,
 * which contain `ROOT::VecOps::RVec<float>` objects. In addition, the argument
 * `object_mask` must point to a column which contains index lists of type
 * `ROOT::VecOps::RVec<int>`. These index lists contain the indices of elements,
 * for which four-vectors should be build. The output column contains a vector
 * of four-vectors `ROOT::VecOps::RVec<PtEtaPhiMVector>` and only contains the
 * elements, which have been selected in by applying the `object_mask`.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the collection vector
 * of Lorentz vectors
 * @param pt name of the column containing the \f$p_T\f$ values of the object
 * collection
 * @param eta name of the column containing the \f$\eta\f$ values of the object
 * collection
 * @param phi name of the column containing the \f$\phi\f$ values of the object
 * collection
 * @param mass name of the column containing the mass values of the object
 * collection
 * @param object_mask name of the mask column indicating selected objects to
 * only calculate the four-vectors for selected objects
 *
 * @return a new dataframe containing the new column
 */
    ...
    auto build_p4_collection =
        [](const ROOT::RVec<float> &pts, const ROOT::RVec<float> &etas,
           ...
           const ROOT::RVec<int> &object_mask) {
            ...
            return ROOT::VecOps::Construct<ROOT::Math::PtEtaPhiMVector>(
                pts_, etas_, phis_, masses_);
        };
    return df.Define(outputname, build_p4_collection,
                     {pt, eta, phi, mass, object_mask});
}
 ...
 *
 * The function expects \f$p_T\f$, \f$\eta\f$, \f$\phi\f$ and mass columns,
 * which contain `ROOT::VecOps::RVec<float>` objects. The output column contains
 * four-vectors `ROOT::VecOps::RVec<PtEtaPhiMVector>`.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the collection vector
 * of Lorentz vectors
...
/**
 * @brief This function is scaling the \f$p_T\f$ and mass (therefore also
 * energy) of a Lorentz vector by a given scale factor.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the scaled Lorentz
 * vector
...
ROOT::RDF::RNode Scale(ROOT::RDF::RNode df, const std::string &outputname,
                       const std::string &vector, const float &scalefactor) {
    return df.Define(outputname,
                     [scalefactor](const ROOT::Math::PtEtaPhiMVector &p4) {
                         if (p4.pt() < 0.0)
                             return ROOT::Math::PtEtaPhiMVector(
                                 default_float, default_float, default_float,
                                 default_float);
