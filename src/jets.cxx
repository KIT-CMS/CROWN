...
/// \param[in] name of the object for debug output (can be also used for electrons)
/// particle candidates
///
/// \return a dataframe containing the new mask
...
VetoJetsByVetoMap(ROOT::RDF::RNode df, 		  const std::string &maskname,
  ...
  auto vetomap_evaluator =
    correction::CorrectionSet::from_file(vetomap_path)
    ->at(vetomap_name);

  auto lambda = [vetomap_evaluator, vetomap_type, logger_str](const ROOT::RVec<float> &eta,
	...
	// due to floating point precision abs(phi) can be >pi, this is ofc not catched in the correction...
	float phi_tmp = phi[j];
	if (phi_tmp > 3.14) {
	  phi_tmp = 3.14;
	}
	...
	veto = bool(vetomap_evaluator->evaluate({vetomap_type, eta_tmp, phi_tmp}));
	/*
	  ╭───────────────────────────── ◀ output ─────────────────────────────╮
	  │ vetomaps (real)                                                    │
	...
	*/
	Logger::get(logger_str)
                    ->debug("checking object with eta {} and phi {}: should object get vetoed? -> {}",
                            eta[j], phi[j], veto);
	mask[j] = 1 - veto;
      }
	Logger::get(logger_str)
                    ->debug("final object veto mask {}",
                            mask);
...
JetPtCorrection(ROOT::RDF::RNode df, const std::string &corrected_jet_pt,
                ...
                const std::string &jec_file, const std::string &jer_tag,
                const std::string &jes_tag, const std::string &jec_algo) {
    Logger::get("JetPtCorrection")
        ->warn("This function without CorrectionManager is deprecated and will "
