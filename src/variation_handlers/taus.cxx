...
GenMatchRestriction::GenMatchRestriction()
    : restrict_(false), gen_matches_({}) {}

GenMatchRestriction::GenMatchRestriction(const std::vector<int> &gen_matches)
    : restrict_(true), gen_matches_(gen_matches) {
    // Validate that only allowed generator-level match values are passed
    for (const auto &gen_match : gen_matches_) {
        if (gen_match != 1 && gen_match != 2 && gen_match != 3 &&
...
bool GenMatchRestriction::is_active() const { return restrict_; }

bool GenMatchRestriction::is_selected(const int &gen_match) const {
    if (!restrict_) {
        return true;
    }
    return (std::find(gen_matches_.begin(), gen_matches_.end(), gen_match) !=
            gen_matches_.end());
}
...
std::string GenMatchRestriction::repr() const {
    ...
    return std::format("GenMatchRestriction(is_active={}, gen_matches=[{}])",
...
DecayModeRestriction::DecayModeRestriction()
    : restrict_(false), decay_modes_(std::vector<int>()) {}

DecayModeRestriction::DecayModeRestriction(const std::vector<int> &decay_modes)
    : restrict_(true), decay_modes_(decay_modes) {
    ...
    for (const auto &decay_mode : decay_modes_) {
        if (decay_mode != 0 && decay_mode != 1 && decay_mode != 10 &&
            decay_mode != 11) {
            auto msg = std::format("Invalid decay mode value: {}. Allowed "
                                   "values are 0, 1, 10, and "
                                   "11.",
                                   decay_mode);
            throw std::invalid_argument(msg);
        }
...
bool DecayModeRestriction::is_active() const { return restrict_; }

bool DecayModeRestriction::is_selected(const int &decay_mode) const {
    if (!restrict_) {
        return true;
    }
    return (std::find(decay_modes_.begin(), decay_modes_.end(), decay_mode) !=
            decay_modes_.end());
}
...
std::string DecayModeRestriction::repr() const {
    ...
    return std::format("DecayModeRestriction(is_active={}, decay_modes=[{}])",
...
PtRestriction::PtRestriction()
    : restrict_(false), pt_range_(std::make_pair(-10.f, -10.f)) {}

PtRestriction::PtRestriction(const float &pt_min, const float &pt_max)
    : restrict_(true), pt_range_(std::make_pair(pt_min, pt_max)) {
    if (pt_range_.first >= pt_range_.second) {
        auto msg = std::format(
            "Invalid pt range: [{}, {}). The lower bound must be smaller than "
            "the upper bound.",
            pt_range_.first, pt_range_.second);
        throw std::invalid_argument(msg);
    }
}

PtRestriction::PtRestriction(const float &pt_min)
    : restrict_(true), pt_range_(std::make_pair(
                           pt_min, std::numeric_limits<float>::infinity())) {
    // No validation needed since infinity is always greater than any finite
    // pt_min
}

bool PtRestriction::is_active() const { return restrict_; }
...
std::string PtRestriction::repr() const {
    return std::format("PtRestriction(is_active={}, pt_range=[{}, {}))",
...
EtaRestriction::EtaRestriction()
    : restrict_(false), abs_eta_range_(std::make_pair(-10.f, -10.f)) {}

EtaRestriction::EtaRestriction(const std::pair<float, float> &abs_eta_range)
    : restrict_(true), abs_eta_range_(abs_eta_range) {
    if (abs_eta_range_.first >= abs_eta_range_.second) {
        auto msg = std::format(
            "Invalid eta range: [{}, {}). The lower bound must be smaller than "
            "the upper bound.",
            abs_eta_range_.first, abs_eta_range_.second);
        throw std::invalid_argument(msg);
    }
}

EtaRestriction::EtaRestriction(const float &abs_eta_min,
                               const float &abs_eta_max)
    : restrict_(true),
      abs_eta_range_(std::make_pair(abs_eta_min, abs_eta_max)) {
    if (abs_eta_range_.first >= abs_eta_range_.second) {
        auto msg = std::format(
            "Invalid eta range: [{}, {}). The lower bound must be smaller than "
            "the upper bound.",
...
bool EtaRestriction::is_active() const { return restrict_; }
...
std::string EtaRestriction::repr() const {
    return std::format("EtaRestriction(is_active={}, abs_eta_range=[{}, {}))",
 ...
 *     sf = correction->evaluate(pt, decay_mode, gen_match, wp, vsele_wp, "up",
 sf_dependence);
 * else {
 *     sf = correction->evaluate(pt, decay_mode, gen_match, wp, vsele_wp, "nom",
 sf_dependence);
 * }
 * ```
 *
 ...
 * std::string wp, vsele_wp, sf_dependence;
 * double sf;
 * sf = correction->evaluate(pt, decay_mode, gen_match, wp, vsele_wp,
...
TauVariationHandler::TauVariationHandler(const std::string &variation)
    ...
    if (is_custom) {
        ...
        if (match_results.find("decay_mode") != match_results.end()) {
            decay_mode_restriction_ =
                DecayModeRestriction(std::stoi(match_results["decay_mode"]));
        }

        // Set pt restriction if matched in the custom variation string
        if (match_results.find("pt_min") != match_results.end() &&
...
TauVariationHandler::wrap_evaluate(
    ...
    auto wrapper = [is_custom_variation = this->is_custom_variation_,
                    ...
                    eta_restriction = this->eta_restriction_, evaluator,
                    gen_index, decay_mode_index, pt_index, eta_index,
                    syst_index](
        ...
        if (gen_match_restriction.is_active()) {
            if (gen_index == static_cast<size_t>(-1)) {
                auto msg = std::format("Variable genmatch not found in the "
                                       "list of inputs of the correction. "
                                       "Please check the variable name and "
                                       "ensure it is present in the "
                                       "correction inputs.");
        ...
        if (decay_mode_restriction.is_active()) {
            if (decay_mode_index == static_cast<size_t>(-1)) {
                auto msg = std::format("Variable dm not found in the list of "
                                       "inputs of the correction. "
                                       "Please check the variable name and "
                                       "ensure it is present in the "
                                       "correction inputs.");
                throw std::out_of_range(msg);
        ...
        if (pt_restriction.is_active()) {
            if (pt_index == static_cast<size_t>(-1)) {
                auto msg = std::format("Variable pt not found in the list of "
                                       "inputs of the correction. "
                                       "Please check the variable name and "
                                       "ensure it is present in the "
                                       "correction inputs.");
        ...
        if (eta_restriction.is_active()) {
            if (eta_index == static_cast<size_t>(-1)) {
                auto msg = std::format("Variable eta not found in the list of "
                                       "inputs of the correction. "
                                       "Please check the variable name and "
                                       "ensure it is present in the "
                                       "correction inputs.");
