 ...
 * It retrieves a per-event weight from a correction file based on the true
 * number of pileup interactions in an event.
 *
 ...
 * pileup weights file
 * @param outputname name of the output column containing the pileup event
 * weight
 * @param true_pileup_number name of the column containing the true mean number
 * of the poisson distribution for an event from which the number of
 * interactions each bunch crossing has been sampled
...
PUWeightROOT(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    if (!datafile || datafile->IsZombie())
        throw std::runtime_error("Could not open data file: " + datafilename);
    if (!mcfile || mcfile->IsZombie())
        throw std::runtime_error("Could not open MC file: " + mcfilename);
    std::shared_ptr<TH1D> datahist((TH1D *)datafile->Get(histname.c_str()));
    ...
    auto puweight = [datahist, mchist](const float truePUMean) {
    ...
    auto df1 = df.Define(outputname, puweight, {truePUMean});
    return df1;
}

/**
 * @brief This function is used to evaluate the parton shower (PS) weight of an
 * event. The weights are stored in the nanoAOD files and defined as
 * \f$w_{variation}\f$ / \f$w_{nominal}\f$. The nominal weight is already
 * applied, therefore, the main use of this function is to get the initial state
 * radiation (ISR) and final state radiation (FSR) variations to the nominal PS
 * weight.
 *
 ...
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the ISR/FSR event
 * weight
 * @param ps_weights name of the column containing the parton shower (ISR/FSR)
 * weights
...
ROOT::RDF::RNode PartonShower(ROOT::RDF::RNode df,
                              ...
                              const std::string &ps_weights, const float isr,
                              const float fsr) {
    ...
    auto ps_weights_lambda = [isr, fsr](const ROOT::RVec<float> ps_weights) {
        if (isr == 1.0 && fsr == 1.0) {
        ...
        if (ps_weights.size() == 4) {
            ...
            Logger::get("event::reweighting::PartonShower")
                ->error("Invalid number of PS weights: {}", ps_weights.size());
    ...
    auto df1 = df.Define(outputname, ps_weights_lambda, {ps_weights});
    return df1;
}

/**
 * @brief This function is used to evaluate the LHE scale weight of an event.
 * The weights are stored in the nanoAOD files and defined as
 * \f$w_{variation}\f$ / \f$w_{nominal}\f$. The nominal weight is already
 * applied, therefore, the main use of this function is to get the factorization
 * and renormalization scale variations to the nominal scale weight.
 *
 ...
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the LHE scale event
 * weight
 ...
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode LHEscale(ROOT::RDF::RNode df, const std::string &outputname,
                          const std::string &lhe_scale_weights,
                          const float mu_r, const float mu_f) {
    // find the index we have to use, first check if the mu_r and mu_f values
    // are valid, only 0.5, 1.0, 2.0 are allowed
    std::vector<float> allowed_values = {0.5, 1.0, 2.0};
    ...
    auto lhe_scale_weights_lambda =
        [mu_r, mu_f](const ROOT::RVec<float> scale_weights) {
            ...
            if (scale_weights.size() == 9) {
                index_map = {{{0.5, 0.5}, 0}, {{1.0, 0.5}, 1}, {{2.0, 0.5}, 2},
...
/**
 * @brief This function is used to evaluate the LHE PDF weight of an event. The
 * weights are stored in the nanoAOD files and defined as \f$w_{variation}\f$ /
 * \f$w_{nominal}\f$. The nominal weight is already applied, therefore, the main
 * use of this function is to get the variation of the PDF weights to the
 * nominal PDF weight.
 *
 * The PDF weights consist of 101 weights, where the first weight is the nominal
 * weight and the remaining 100 weights correspond to alternative PDF sets.
 *
 * @note The proper procedure is to use each alternative PDF set as an
 * independent systematic vatiation. However, in case of this function, a
 * simplified approach is used to calculate a single PDF weight variation. The
 * standard deviation of the 100 alternative PDF weights is calculated and used
 * to define the up and down variations as follows: \f$w_{up/down} = 1 \pm
 * \sqrt{\sum_{i=1}^{100} (w_i - 1)^2}\f$
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the LHE PDF event
 * weight
 * @param lhe_pdf_weights name of the column containing the LHE PDF weights
 * @param variation name of the variation that should be evaluated, possible
 * values are "nominal", "up", "down"
 *
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode LHEpdf(ROOT::RDF::RNode df, const std::string &outputname,
            ...
            if (n_pdfs == 101 || n_pdfs == 103) {
                ...
                if (variation == "up") {
                    ...
                    throw std::runtime_error(
                        "Invalid variation for LHE PDF weights");
                }
            } else {
                Logger::get("event::reweighting::LHEpdf")
    ...
    auto df1 = df.Define(outputname, lhe_pdf_weights_lambda, {lhe_pdf_weights});
    return df1;
}

/**
 * @brief This function is used to evaluate the LHE \f$\alpha_S\f$ weight of an
 * event. The weights are stored in the nanoAOD files and defined as
 * \f$w_{variation}\f$ / \f$w_{nominal}\f$. The nominal weight is already
 * applied, therefore, the main use of this function is to get the variation of
 * the \f$\alpha_S\f$ weight to the nominal weight.
 *
 * For some samples the \f$\alpha_S\f$ weight is included in the PDF weights
 * vector. In that case the full PDF weights vector is expected to contains 103
 * entries, where the first 101 entries are PDF weights and the last two entries
 * correspond to the up and down varied \f$\alpha_S\f$ weight.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the LHE \f$\alpha_S\f$
 * event weight
 * @param lhe_pdf_weights name of the column containing the LHE \f$\alpha_S\f$
 * weights (it is part of the LHE PDF weights)
 * @param variation name of the variation that should be evaluated, possible
 * values are "nominal", "up", "down"
 *
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode LHEalphaS(ROOT::RDF::RNode df, const std::string &outputname,
                           const std::string &lhe_pdf_weights,
                           const std::string &variation) {
    auto lhe_alphaS_weights_lambda = [variation](
                                         const ROOT::RVec<float> pdf_weights) {
        // the nominal weight is already applied, so we can return 1.0
        if (variation == "nominal") {
            return (float)1.0;
        }
 ...
 */
ROOT::RDF::RNode TopPt(ROOT::RDF::RNode df, const std::string &outputname,
 ...
 * mismodeled compared to data. This function is defined for the corrections
 * provided by the CMS HLepRare group. More details can be found here:
 ...
 * file. Otherwise the specific uncertainty source "X" is used (where X is a
 * number e.g. 1,2,3,...).
 *
 * @return a new dataframe containing the new column
 */
...
ZBosonPt(ROOT::RDF::RNode df,
    ...
    auto corrDYunc = correction_manager.loadCorrection(
        corr_file, corr_name + "_N_uncertainty");

    // Get number of uncertainties
    auto n_unc = corrDYunc->evaluate({order});

    // Get weight depending on variation
    auto corr = [corrDY, order, variation,
                 n_unc](const ROOT::Math::PtEtaPhiMVector &gen_boson) {
        float weight = 1.0;
        if (variation == "up" || variation == "down") {
            float weight_nom =
                corrDY->evaluate({order, (float)gen_boson.Pt(), "nom"});
            weight = weight_nom;

            for (int i = 1; i <= n_unc; i++) {
                float unc_up = corrDY->evaluate(
                    {order, (float)gen_boson.Pt(), "up" + std::to_string(i)});
                float unc_down = corrDY->evaluate(
                    {order, (float)gen_boson.Pt(), "down" + std::to_string(i)});

                if (variation == "up" && unc_up > weight_nom &&
                    unc_up > weight) {
                    weight = unc_up;
                } else if (variation == "up" && unc_down > weight_nom &&
                           unc_down > weight) {
                    weight = unc_down;
                } else if (variation == "down" && unc_down < weight_nom &&
                           unc_down < weight) {
                    weight = unc_down;
                } else if (variation == "down" && unc_up < weight_nom &&
                           unc_up < weight) {
                    weight = unc_up;
                }
            }
        } else if (variation == "nom" ||
                   variation.rfind("up") != std::string::npos ||
                   variation.rfind("down") != std::string::npos) {
            weight =
                corrDY->evaluate({order, (float)gen_boson.Pt(), variation});
        } else {
 ...
 * for the legacy \f$H(\tau\tau)\f$ analysis and therefore not up-to-date
 * anymore for UL or Run3.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the derived event
 * weight
 * @param gen_boson name of the column containing the Lorentz vector of the
 * generator-level boson
 * @param workspace_file path to the file which contains the workspace that
 * should be used
