...
// Due to unknowns reasons, this function must be implemented inline in the header file, otherwise the linker will complain about undefined references. Moving the implementation to the source file will result in a linker error. Why, I don't know...
...
PNNEvaluate_ORT(ROOT::RDF::RNode df, OnnxSessionManager &onnxSessionManager,
    ...
    bool found_col = (std::find(col_names.begin(), col_names.end(), "event_id") != col_names.end());
    auto df1 = df;
    if (!found_col) {
        df1 = df.Define(
            "event_id", [](unsigned long long event) { return float(event); }, {"event"});
    }
    ...
    auto NNEvaluator = [session_odd, session_even, allocator, input_node_dims, output_node_dims,
                        num_input_nodes,
        ...
        if (event_id%2 == 0) {
            output = onnxhelper::run_interference(
                session_odd, allocator, inputs, input_node_dims, output_node_dims,
                num_input_nodes, num_output_nodes);
        }
        else {
            output = onnxhelper::run_interference(
                session_even, allocator, inputs, input_node_dims, output_node_dims,
                num_input_nodes, num_output_nodes);
        }
    ...
    auto df2 = df1.Define(outputname_vector, utility::PassAsVec<(nParameters+1), float>(NNEvaluator), InputList);
