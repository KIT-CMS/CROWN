...
class GraphParser:
    ...
    def parse_from_call(self, producer, parent, scope, align=""):
        ...
        if not input_vec_config in producer.vec_configs:
            raise Exception(f"Input name from {call} not in producer vector configs {producer.vec_configs}")
        else:
    ...
    def generate_graph(self):
        ...
        for scope in self.config.scopes:
            ...
            for p in self.config.producers[scope]:
                self.parse_producer(producer=p, parent=f"scope", scope=scope, align="    ")
            ...
            self.add_node(id_name=self.out_file_name, name=f"{scope}_{self.out_file_name}", scope=scope, node_type="I/O")
        ...
        for scope in self.config.scopes:
            ...
            for key in all_keys:
                ...
                if len(set(total_output_nodes)) != 1:
                    raise Exception(f"Output {output} has multiple origins: {total_output_nodes}")
        self.assemble_edges()
    ...
    def parse_producer(self, producer, parent, scope, align=""):
        if producer.__class__.__name__ in ["VectorProducer"]:
            if self.debugging:
                print(align + f"Adding VectorProducer: {producer.name}")
            print(f"!!!{producer} is a lagacy producer and should be replaced with ExtendedVectorProducer!!!")
        ...
        elif producer.__class__.__name__ in ["ExtendedVectorProducer"]:
            if self.debugging:
                print(align + f"Adding VectorProducer: {producer.name}")
            group_id =f"{producer.name}_v"
            self.add_node(id_name=group_id, name=producer.name, scope=scope, parent=parent, node_type="vector")
        ...
        elif producer.__class__.__name__ in ["Filter"]:
            if self.debugging:
        ...
        elif producer.__class__.__name__ in ["BaseFilter"]:
            if self.debugging:
                print(align + f"Adding Filter: {producer.name}")
