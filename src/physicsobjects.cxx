...
/**
 * @brief This function defines a mask in the dataframe that selects objects
 * based on eta-dependent upper and lower thresholds for a given object
 * `quantity`. The selection criteria differ between the barrel and endcap
 ...
 * @param lower_threshold_barrel lower threshold for selection in the barrel
 * region
 * @param upper_threshold_barrel upper threshold for selection in the barrel
 * region
 * @param lower_threshold_endcap lower threshold for selection in the endcap
 * region
 * @param upper_threshold_endcap upper threshold for selection in the endcap
 * region
 *
...
/**
 * @brief This function updates the mask by setting the value at the specified
 * index to zero, effectively vetoing the object at that position.
 *
 * @param df input dataframe
 * @param outputname name of the output mask column after vetoing the object
 * @param object_mask name of the mask column that contains object selection
 * flags
...
ROOT::RDF::RNode VetoSingleObject(ROOT::RDF::RNode df,
    ...
    return df.Define(
        ...
        [index, object_mask](const ROOT::RVec<int> &mask) {
            Logger::get("physicsobject::VetoSingleObject")
 ...
 * @param position position in the index vector that specifies which object to
 * veto
 *
 * @return a dataframe containing the new mask as a column
 */
...
/**
 * @brief This function counts the number of selected objects in the provided
 * object mask and stores the number.
 *
 * @param df input dataframe
...
/**
 * @brief This function checks how many objects are selected based on the
 * provided mask and returns `true` if the count matches the specified number.
 *
 * @param df input dataframe
...
/**
 * @brief This function checks if there is at least one selected object in the
 * provided object mask. If at least one object is selected, the output flag is
...
/**
 * @brief This function checks if any object in a given collection
 * (`object_mask`) overlaps with a specified target object defined by its
 * four-momentum within a user-defined `delta_r_cut` distance. If an object is
 * found within this threshold, the function returns `true`, indicating an
 * overlap with the target object. The result can be used to veto events that
 ...
 * @param object_mask name of the column containing the mask of selected objects
 * to compare with
 * @param min_delta_r minimal deltaR distance allowed between objects and target
...
OverlapVeto(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto veto_overlapping_target = [min_delta_r](
                                       ...
                                       const ROOT::RVec<int> &mask) {
        ROOT::RVec<int> selected_indices = ROOT::VecOps::Nonzero(mask);
        const auto selected_pts = ROOT::VecOps::Take(pts, selected_indices);
        const auto selected_etas = ROOT::VecOps::Take(etas, selected_indices);
        const auto selected_phis = ROOT::VecOps::Take(phis, selected_indices);
    ...
    auto df1 = df.Define(outputname, veto_overlapping_target,
                         {target_p4, object_pt, object_eta, object_phi,
                          object_mass, object_mask});
    return df1;
}
...
LeptonPairVeto(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto pair_finder_lambda = [max_delta_r](const ROOT::RVec<float> &pts,
                                            const ROOT::RVec<float> &etas,
                                            const ROOT::RVec<float> &phis,
                                            const ROOT::RVec<float> &masses,
        ...
        for (auto index_1 = selected_indices.begin();
            ...
            for (auto index_2 = index_1 + 1; index_2 != selected_indices.end();
                 index_2++) {
                if (charges.at(*index_1) != charges.at(*index_2)) {
                    auto p4_1 = ROOT::Math::PtEtaPhiMVector(
                        pts.at(*index_1), etas.at(*index_1), phis.at(*index_1),
                        masses.at(*index_1));
                    auto p4_2 = ROOT::Math::PtEtaPhiMVector(
                        pts.at(*index_2), etas.at(*index_2), phis.at(*index_2),
                        masses.at(*index_2));
                    if (ROOT::Math::VectorUtil::DeltaR(p4_1, p4_2) >=
                        max_delta_r)
                        return true;
                }
            }
...
/**
 * @brief This function extracts the indices of objects that are marked as
 * selected in the given object mask.
 *
 * @param df input dataframe
...
ROOT::RDF::RNode GetIndices(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    return df.Define(outputname,
                     [](const ROOT::RVec<int> &mask) {
                         ...
                         Logger::get("physicsobject::GetIndices")
...
ROOT::RDF::RNode OrderByPt(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(
        ...
        [outputname, object_mask](const ROOT::RVec<float> &pts,
                                  const ROOT::RVec<int> &mask) {
            Logger::get("physicsobject::OrderByPt")
                ->debug(
                    "Ordering good objects from {} by pt, output stored in {}",
                    object_mask, outputname);
            Logger::get("physicsobject::OrderByPt")
                ->debug("Object pt before {}", pts);
            Logger::get("physicsobject::OrderByPt")->debug("Mask {}", mask);
            auto good_object_pts =
                ROOT::VecOps::Where(mask > 0, pts, (float)0.);
            Logger::get("physicsobject::OrderByPt")
                ->debug("Object pt after {}", good_object_pts);
...
ROOT::RDF::RNode MassCorrectionWithPt(ROOT::RDF::RNode df,
    ...
    auto mass_correction_lambda = [](const ROOT::RVec<float> &masses,
                                     const ROOT::RVec<float> &pts,
                                     const ROOT::RVec<float> &corrected_pts) {
        ROOT::RVec<float> corrected_masses(masses.size());
        for (int i = 0; i < masses.size(); i++) {
            corrected_masses[i] =
                masses.at(i) * corrected_pts.at(i) / pts.at(i);
        }
