...
/** 
 * @brief Function to apply a flag filter to the dataframe. The input flag can be an 
 * already existing flag in the NanoAOD like e.g. the noise filters recommended by 
 * the CMS JetMET group (https://twiki.cern.ch/twiki/bin/viewauth/CMS/MissingETOptionalFiltersRun2).
...
/**  
 * @brief Function to apply a selection on a quantity of type T (e.g. int, float, 
 * double, bool). The type has to be defined when calling this function 
 * `event::QuantityFilter<T>`. If the quantity value is in the given selection 
 * value vector (with values of type T), an event is kept. If not it is filtered out.
 *
 * @param df input dataframe
 * @param filtername name of the filter, used in the dataframe report
 ...
 */
template <typename T>
inline ROOT::RDF::RNode QuantityFilter(ROOT::RDF::RNode df,
                                    const std::string &filtername,
...
/** 
 * @brief This function creates a new column `outputname` with the negatives of the values 
 * in the column `inputname`.
 *
 * Note that this function is implemented as a template, so specify the type `T` of the objects 
 * in the input column when calling this function with `Negative<T>(...)`. The type can also be 
 * a vector with `ROOT::VecOps::RVec<T>`.
 *
 * @param df input dataframe
 ...
 */
template <typename T>
inline ROOT::RDF::RNode GetQuantity(ROOT::RDF::RNode df, const std::string &outputname,
...
/** 
 * @brief Function to sum all elements of the column with name `quantity` containing 
 * `ROOT::VecOps::RVec<T>` objects.
 *
 * This function is a template implementation, i.e., call SumVectorQuantity<T> if the column
 * `quantity` contains `ROOT::VecOps::RVec<T>` objects.
 *
 * Elements of the `ROOT::VecOps::RVec<T>`, which should enter the sum, can be selected 
 * with index lists from the column `collection_index` as `ROOT::VecOps::RVec<int>` objects 
 * per entry.
 *
 * Internally, `ROOT::VecOps::Sum` is used to calculate the sum. A custom zero element, 
 * which is a second optional argument of `ROOT::VecOps::Sum`, can be passed to this function 
 * by setting the parameter `zero`. Its default value is `T(0)`. For instance, when dealing 
 * with `ROOT::Math::PtEtaPhiMVector` objects, the `zero` parameter must be set to 
 ...
 * @param collection_index column name for index lists of the elements to be summed
 * @param zero zero element passed as the second argument to the `ROOT::VecOps::Sum` function
 *
 * @return a dataframe with the new column
 */
template <typename T>
inline ROOT::RDF::RNode SumVectorQuantity(ROOT::RDF::RNode df,
    ...
    auto sum_per_event = [zero](const ROOT::RVec<T> &quantity, const ROOT::RVec<int> &collection_index) {
        Logger::get("SumVectorQuantity")->debug(
 ...
 * Internally, `ROOT::VecOps::Sum` is used to calculate the sum. A custom zero element, 
 * which is a second optional argument of `ROOT::VecOps::Sum`, can be passed to this function 
 * by setting the parameter `zero`. Its default value is `T(0)`.
 *
 * @param df input dataframe
 * @param outputname name of the output column
 * @param quantity column name of the vector quantity which is summed per event
...
/**  
 * @brief Helper function to recursively define columns for each entry of a vector
 * quantity.
 *
 * @param df input dataframe
 ...
 * this function
 *
 * @return a lambda function to be used in RDF Define and a new column for each vector entry
 */
template <typename T>
...
/** 
 * @brief This function defines a flag that is true if at least one of the input flags is true.
 *
 * @param df input dataframe
