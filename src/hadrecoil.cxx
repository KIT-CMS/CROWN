...
ROOT::RDF::RNode scalar_ht(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto scalar_ht_func = [position](const ROOT::RVec<int> &jet_pt, const ROOT::RVec<int> &jet_collection
    const std::string &outputname) {
        // add up the magnitudes of the pt of all selected jets
        float ht = 0;
        for (const int &jet_index : jet_collection) {
            ht += jet_pt_corrected.at(jet_index);
        }
        return ht;
    }
...
ROOT::RDF::RNode vectorial_mht(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto vectorial_mht_func = [position](const ROOT::RVec<int> &jet_pt, const ROOT::RVec<int> &jet_eta, const ROOT::RVec<int> &jet_collection) {
