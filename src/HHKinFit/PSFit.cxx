...
int PSFit::PSfitter(int iloop, int &iter, int &method, int &mode,
               bool &noNewtonShifts, int np, double a[],
               double a_start[], double a_limit[][2], double a_precision[],
               double daN[], double h[], double a_Memory[][5], double chi2,
               double chi2_iter[], double g[], double H[], double H_inv[])
{
    ...
    // chi2_iter[]  Function (chi2) with nominal a[], i.e. no shift of a[]
    ...
    if (iloop == 0) {                                       // start of a new fit
        ...
        for (int ip = 0; ip < np; ip++) {                          // check limits
            ...
            if (a_start[ip] < a_limit[ip][0] || a_start[ip] > a_limit[ip][1]) { // NOT WORKING astart has no effect once the loop started!
                a_start[ip] = 0.5 * (a_limit[ip][0] + a_limit[ip][1]); // set to mean in between limits
                return convergence;
            }
        }
    }
    ...
    if (method == 1) {                // ----------------------------- line search
        ...
        if (mode == 1) {                     // start of a new line search
            ...
            //      x_h     = PSVnorm(h, np) ;        // initial step width for line search
        ...
        if (mode <= 0) {                                            // Minimum found
            ...
            // std::cout << "Eb1 = " << a_start[0] + x[1] * daN[0] << " ETau1 = " << a_start[1] + x[1] * daN[1] << std::endl;
            // std::cout << "and" << std::endl;
            // std::cout << "Eb1 = " << a_start[0] + x[3] * daN[0] << " ETau1 = " << a_start[1] + x[3] * daN[1] << std::endl;
            ...
            for (int ip = 0; ip < np; ip++) { // check for progress w.r.t. previous iteration
                if (std::abs(a[ip] - a_Memory[ip][0]) > a_precision[ip]) {
                    // std::cout << "No Convergence as Parameter " << np << " is " << std::abs(a[ip] - a_Memory[ip][0]) << " away from previous iteration." << std::endl;
                    didConverge = false;
                    break;
                }
            }
            ...
            for (int previous_iter = 1; previous_iter <= 4; previous_iter++) {
                ...
                for (int ip = 0; ip < np; ip++) {
                    if(std::abs(a_Memory[ip][previous_iter] - a[ip]) > a_precision[ip]*0.01) {
                ...
                if(we_have_been_here_before) {
                    // std::cout << "Pattern recognized! We have been here " << previous_iter+1 << " iterations ago! Disable NewtonShifts!" << std::endl;
                    noNewtonShifts = true;
                }
            }
    ...
    if (method == 2) {                                    // Newton method
        ...
        if (ready == 1) {
            ...
            PSNewtonLimitShift(-1, np, a, a_limit, a_precision, daN, h, g, H);
            d = PSNewtonAnalyzer(np, a, a_limit, a_precision, daN, h, g, H, H_inv, chi2, noNewtonShifts);

            if (d < eps_x) {      // test convergence in next run by checking if delta_Chi2 is small enough
                method = 3;
...
void PSFit::PSLineLimit(int np, double a_start[], double daN[], double a_limit[][2], double x_limit[])
{
...
double PSFit::PSLineSearch(int &mode, double hh, double x_limit[],
...
{      // 1-dim Line-Search, Method from Blobel textbook p. 252
    ...
    if (mode == 1) {
        ...
        h = std::min(std::abs(hh), 0.25 * (x_limit[1] - x_limit[0]));  // limit h to x_limit
        x[1] = 0.;
        f[1] = chi2;
        x[2] = x[1] + h;
        ...
        if ((x[1] > x[2] && x[1] < x[3]) || (x[1] < x[2] && x[1] > x[3])) {    //Local maximum between x[2] and x[3]! Continue in direction of steepest descent.
            ...
            if ((f[1] - f[2]) > (f[1] - f[3])) {  //Continue in direction of x[2]
                temp = x[3];
                x[3] = x[2] + x[2] - x[1];
                if ((x[3] > x_limit[1] - eps_x) || (x[3] < x_limit[0] + eps_x)) {      //To close to limit. Other direction.
                    x[3] = temp;
                    x[2] = x[3];
                    f[2] = f[3];
                    x[3] = x[2] + x[2] - x[1];
        ...
        else if (std::abs(x[1] - x[2]) > std::abs(x[1] - x[3])) {  // exchange x2 <-> x3
            temp = f[3];
            f[3] = f[2];
            f[2] = temp;
            temp = x[3];
        ...
        if (f[3] <= f[2]) { //  no minimum found:  shift x1 <- x2 <- x3 and get next x3
            ...
            if (i_limit > -1) {                 // limit x3 to x_limit0 <= x3 <= x_limit1
                if (fabs(x[2] - x_limit[i_limit]) < eps_x) { // Minimum found at limit : finish !
        ...
        if ((std::abs(x[1] - x[3]) < eps_x && delta_Chi2 < eps_f) || std::abs(x[1] - x[3]) < 0.00001) {// CONVERGENCE reached   Requirement of small delta_Chi2 difficult in very steep regions
            mode = 0;
            return x[2];
        }
        ...
        if (std::abs(x[2] - xt) < close * d31) {       // safety for numerical precision
            if (d21 < d32) {
                xt = x[2] + tau_fac * d31;
...
int PSFit::PSderivative(int icall, int np, double a[], double h[],
                      double chi2, double chi2_iter[], double g[], double H[])
{
    ...
    if (ia_old >= 0) {
        ...
        H[ia_old * np + ia_old] = H[ia_old * np + ia_old] + chi2; // build up Hesse diagonal
    }
    if (ia_i_old >= 0) {
        H[ia_i_old * np + ia_j_old] = H[ia_i_old * np + ia_j_old] + chi2; //Hesse (non-diagonal)
    }
    if (icalc == shift_mm2) {
        for (int ii = 0; ii < np; ii++) {
        ...
        for (int ii = 0; ii < np - 1; ii++) {                 // Hesse non-diagonal
            for (int jj = ii + 1; jj < np; jj++) {
                H[ii * np + jj] = H[ii * np + jj] - pow(h[ii], 2.0) * H[ii * np + ii] - pow(h[jj], 2.0) * H[jj * np + jj];
            };
        };
...
int PSFit::PSderivative1(int icall, double a[], double h[], double F, double g[], double H[])
{
...
double PSFit::PSNewtonAnalyzer(int np, double a[], double a_limit[][2],
                          ...
                          double g[], double H[], double H_inv[], double chi2, bool noNewtonShifts)
{
    ...
    for (int ip = 0; ip < np; ip++) {    // check for positive Diagonal of Hesse
        if (H[ip * np + ip] < 0.) {
            iNewton = 0;
            // std::cout << "PSNewton Analyzer check 2 (negative diagonal entries)" << std::endl;
            break;
        }
    ...
    // std::cout << "daN shift for Newton in daN[0]: " << daN[0] << " daN[1]: " << daN[1] << std::endl;
    
    double x_max = std::min(1., x_limit[1]); //  Newton step would lead out of limits

    for (int ip = 0; ip < np; ip++) {                       // apply Newton step
        a[ip] = a[ip] + daN[ip] * x_max;                  // update parameter value
        h[ip] = 100. * sqrt(0.000001 * std::abs(chi2 / H[ip * np + ip])); // update step width
    }

    // std::cout << "Newton shifted to: a[0] = " << a[0] << " a[1] = " << a[1] << std::endl;
...
double PSFit::PSMinverse(double H[], double H_inv[], int p)
{    // invert a symmetric matrix H[p,p]
    ...
    if (p == 2) {
        ...
        H_inv[0 * p + 0] = H[1 * p + 1] * H[2 * p + 2] - H[1 * p + 2] * H[2 * p + 1];
        H_inv[0 * p + 1] = H[1 * p + 2] * H[2 * p + 0] - H[1 * p + 0] * H[2 * p + 2];
        H_inv[0 * p + 2] = H[1 * p + 0] * H[2 * p + 1] - H[1 * p + 1] * H[2 * p + 0];
        H_inv[1 * p + 0] = H[2 * p + 1] * H[0 * p + 2] - H[2 * p + 2] * H[0 * p + 1];
        H_inv[1 * p + 1] = H[2 * p + 2] * H[0 * p + 0] - H[2 * p + 0] * H[0 * p + 2];
        H_inv[1 * p + 2] = H[2 * p + 0] * H[0 * p + 1] - H[2 * p + 1] * H[0 * p + 0];
        H_inv[2 * p + 0] = H[0 * p + 1] * H[1 * p + 2] - H[0 * p + 2] * H[1 * p + 1];
        H_inv[2 * p + 1] = H[0 * p + 2] * H[1 * p + 0] - H[0 * p + 0] * H[1 * p + 2];
        H_inv[2 * p + 2] = H[0 * p + 0] * H[1 * p + 1] - H[0 * p + 1] * H[1 * p + 0];
        double det = H[0 * p + 0] * H_inv[0 * p + 0] + H[1 * p + 0] * H_inv[0 * p + 1] + H[2 * p + 0] * H_inv[0 * p + 2];
...
double PSFit::PSMCholesky(double M[], double R[], int n)
...
double PSFit::PSMRTrianInvert2(double R[], int n)
...
double PSFit::PSMmultiplyMRRT(double A[], int n1, int n2)
