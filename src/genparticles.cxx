...
ROOT::RDF::RNode fatjet_mu_tau_deltaR(ROOT::RDF::RNode df, const std::string &outputname,
                             const std::string &genparticles_pdgid,
                        ...
                        if (abs(pdgids[i]) == 15 && (statusFlags[i] & (1 << 13))  ) { // Direct tau decay product
                            ...
                            for (size_t k = 0; k < pdgids.size(); ++k) {
                                if (motherIdx[k] == i) {
                                    ...
                                    Logger::get("fatjet_mu_tau_deltaR")
                            ...
                            for (size_t j = 0; j < pdgids.size(); ++j) {
                                ...
                                if (abs(pdgids[j]) == 15 && motherIdx[j] == motherIdx[i] && (statusFlags[j] & (1 << 13))  ) {
                                    ...
                                    if (isMuonicTau) {
                                        ...
                                        // Compute DeltaR between the fatjet and the combined tau 4-vector
                                        if (fatjet_p4.Pt() !=-10 ){
                                        float deltaR = ROOT::Math::VectorUtil::DeltaR(fatjet_p4, tau_p4_sum);
                                        minDeltaR = deltaR;

                                        Logger::get("fatjet_mu_tau_deltaR")
                                            ->debug("We have a mu tau_h pair deltaR between fatjet and mu tau pair {}", deltaR);
                                        }
                                    }
                                }
                            }
                            ...
                            for (size_t j = 0; j < pdgids.size(); ++j) {
                                ...
                                if (abs(pdgids[j]) == 15 && motherIdx[j] == motherIdx[i] && (statusFlags[j] & (1 << 13))  ) {
                                    ...
                                    if (isMuonicTau) {
                                        ...
                                        // Compute DeltaR between the hadronic and the muonic tau 4-vector
                                        if (fatjet_p4.Pt() !=-10 ){
                                        float deltaR = ROOT::Math::VectorUtil::DeltaR(hadronicTau, muonicTau);
                                        minDeltaR = deltaR;

                                        Logger::get("gen_mu_tau_deltaR")
                                            ->debug("We have a mu tau_h pair, delta R between muonic and hadronic tau{}", deltaR);
                                        }
                                    }
                                }
                            }
