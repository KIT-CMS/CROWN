...
/**
 * @brief Function to evaluate the di-tau trigger or etau/mutau cross trigger scale factor for embedded events from a xpog file
 *
 * @param df the input dataframe
 * @param pt the name of the column containing the tau pt variable
 * @param decaymode the name of the column containing the tau decay mode
 ...
 * @param type the type of the tau trigger, available are "ditau", "etau", "mutau", "ditauvbf"
 * @param corrtype name of the tau trigger correction type, available are "eff_data", "eff_mc", "sf"
...
ditau_trigger_sf(ROOT::RDF::RNode df, const std::string &pt, 
    ...
    // tauTriggerSF is the only correction set in the file for now, might change with official sf release -> change into additional input parameter 
    auto evaluator =
