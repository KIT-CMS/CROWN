 ...
 * @param variation_barrel name of the scale factor variation for the barrel
 * region
 * (\f$|\eta| <1.46\f$), "nom" for nominal and "up"/"down" the up/down variation
 * @param variation_endcap name of the scale factor variation for the endcap
 * region
 * (\f$1.558 \leq |\eta| <2.3\f$), "nom" for nominal and "up"/"down" the up/down
 * variation
 *
 * @return a new dataframe containing the new column
 *
 * @note This function is intended to be used with Run 2 analyses. The scale
 * factor additionally depends on the decay mode of the tau in Run 3. For Run 3
 * analyses, use the overloaded version of this function.
 */
ROOT::RDF::RNode
    ...
    auto sf_calculator = [evaluator, wp, variation_barrel, variation_endcap,
        ...
        if (eta > -5.0) {
            ...
            if (std::abs(eta) < max_abs_eta_barrel) {
                sf =
                    evaluator->evaluate({eta, gen_match, wp, variation_barrel});
            } else if (std::abs(eta) >= min_abs_eta_endcap &&
                       std::abs(eta) < max_abs_eta_endcap) {
                sf =
                    evaluator->evaluate({eta, gen_match, wp, variation_endcap});
            } else {
                sf = 1.;
            }
        }
 ...
 * correctionlib file using a specified scale factor name and variation. The
 * variation and the scale factor itself is binned in pseudorapidities of
 * hadronic taus for this function. This function is intended to be used with
 * Run 3 analyses.
 *
 * Description of the bit map used to define the tau ID against electrons
 * working points of the DeepTau v2.5 tagger. vsElectrons |  ID value (value
 * used in the config)
 * ------------------------------------|--------
 ...
 * @return a new dataframe containing the new column
 *
 * @note This function is intended to be used with Run 3 analyses. The scale
 * factor additionally depends on the decay mode of the tau. For Run 2 analyses,
...
Id_vsEle(ROOT::RDF::RNode df,
         ...
         const std::string &decay_mode, const std::string &gen_match,
         const std::string &sf_file, const std::string &sf_name,
         const std::string &wp, const std::string &variation_barrel,
         const std::string &variation_endcap) {
    Logger::get("physicsobject::tau::scalefactor::Id_vsEle")
        ->debug("Setting up function for tau id vsEle sf");
    Logger::get("physicsobject::tau::scalefactor::Id_vsEle")
