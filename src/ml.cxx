...
namespace ml {
/**
 * @brief Function to perform a standard transformation of input variables for NN evaluation.
 *
 * @param df The input dataframe
 * @param inputname name of the variable which should be transformed
 * @param outputname name of the output column
 * @param param_file path to a json file with a dictionary of mean and std values
 * @param var_type variable data type for correct processing e.g. "i" for integer or "f" for float
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode
StandardTransformer(ROOT::RDF::RNode df, const std::string &inputname,
                  const std::string &outputname, const std::string &param_file, const std::string &var_type) {
    // read params from file
    Logger::get("StandardTransformer")->debug("reading file {}", param_file);
    std::string replace_str = std::string("EVTID");
    std::string odd_file_path = std::string(param_file).replace(param_file.find(replace_str), replace_str.length(), std::string("odd"));
    std::string even_file_path = std::string(param_file).replace(param_file.find(replace_str), replace_str.length(), std::string("even"));
    ...
    // odd or even files mean that they are trained on odd or even events, so it has to be applied on the opposite
    auto transform_int = [odd_info, even_info, inputname](const unsigned long long event_id, const int input_var) {
        float shifted = -10;
        if (int(event_id)%2==0) {
            shifted = (float(input_var) - float(odd_info[inputname]["mean"])) / float(odd_info[inputname]["std"]);
        }
        else if (int(event_id)%2==1) {
            shifted = (float(input_var) - float(even_info[inputname]["mean"])) / float(even_info[inputname]["std"]);
        }
        Logger::get("StandardTransformer")
            ->debug("transforming var {} from {} to {}", inputname, input_var, shifted);
        return shifted;
    }; 
    auto transform_float = [odd_info, even_info, inputname](const unsigned long long event_id, const float input_var) {
        float shifted = -10;
        if (int(event_id)%2==0) {
            shifted = (float(input_var) - float(odd_info[inputname]["mean"])) / float(odd_info[inputname]["std"]);
        }
    ...
    if (var_type.rfind("i", 0) == 0) {
        ...
        auto df1 = df.Define(outputname, transform_float, {event_id, inputname});
        return df1;
    }
    else {
        Logger::get("StandardTransformer")
            ->debug("transformation failed due to wrong variable type: {}", var_type);
        return df;
    }
}

/**
 * @brief Function to produce columns with mass values for the evaluation of parametric neural networks for the NMSSM analysis.
 *
 * @param df The input dataframe
 * @param outputname name of the output column
 * @param mass_paramfile path to a json file with a dictionary for mass value transformations
 * @param mass_parameter actual mass value to be transformed 
 * @param boson one of the two boson with unknown masses "X" or "Y"
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode DefineMassColumns(ROOT::RDF::RNode df, const std::string &outputname, const std::string &mass_paramfile, const std::string &mass_parameter, const std::string &boson) {
...
namespace sofie {
/**
 * @brief Function which includes the model header file and defines a functor for the evaluation in RDFs.
 *
...
void CompileModelForRDF(const std::string &headerModelFile,
    ...
    Logger::get("SOFIEGenerator")->debug("Compiled custom SOFIE helpers: {}", cmd); 

    cmd = "auto sofie_functor_" + modelName +
          " = SofieFunctor<" + std::to_string(ninputs) +
          ",TMVA_SOFIE_" + modelName + "::Session>(" + std::to_string(nslots) +
          ");";
    ret = gInterpreter->Declare(cmd.c_str());
    if (!ret) {
        throw std::runtime_error("Error compiling : " + cmd);
...
std::string SOFIEGenerator(std::vector<std::string> input_vec,
    ...
    if (!std::filesystem::exists(modelHeaderFile)) {
        ...
        TMVA::Experimental::SOFIE::RModel model = parser.Parse(model_file, verboseParser);
...
/**
 * @brief Function for the evaluation of parametric neural networks for the NMSSM analysis.
 *
 * @param df The input dataframe
 * @param input_vec vector of input variables to the model
 * @param outputname_vector name of the output column with the full prediction vector
 * @param outputname_class name of the output column with the class with the highest value
 ...
 */
ROOT::RDF::RNode NMSSMEvaluate_ONNX(ROOT::RDF::RNode df,
                                 const std::vector<std::string> &input_vec,
                                 const std::string &outputname_vector,
                                 const std::string &outputname_class,
    ...
    Logger::get("NMSSMEvaluate")->debug("mass hypothesis: mX={}, mY={}", massX_parameter, massY_parameter);
    ...
    std::string replace_str = std::string("EVTID");
    std::string model_file_path_odd = std::string(model_file).replace(model_file.find(replace_str), replace_str.length(), std::string("odd"));
    std::string model_file_path_even = std::string(model_file).replace(model_file.find(replace_str), replace_str.length(), std::string("even"));
    ...
    Logger::get("NMSSMEvaluate")->debug("evaluating model code with: {} and {}", eval_func_even, eval_func_odd);

    std::string eval_func = "(event%2==0) ? " + eval_func_odd + " : " + eval_func_even + ";";

    auto df1 = df.Define(outputname_vector, eval_func);
    auto df2 = df1.Define(
        outputname_class, 
        [](const std::vector<float> &prediction) {
            int max_idx = std::distance(prediction.begin(), std::max_element(prediction.begin(), prediction.end()));
            return max_idx;
        },
        {outputname_vector});
    auto df3 = df2.Define(
