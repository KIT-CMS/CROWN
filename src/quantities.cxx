 ...
 * where \f$\eta_1\f$ and \f$\phi_1\f$ are from the first Lorentz vector and
 * \f$\eta_2\f$ and \f$\phi_2\f$ are from the second Lorentz vector.
 *
 * @param df input dataframe
...
ROOT::RDF::RNode PairHemisphere(ROOT::RDF::RNode df,
    ...
    return df.Define(outputname, calculate_hemisphere, {vector_1, vector_2});
}

/**
 * @brief This function calculates the quantity `pZetaMissVis` from the two
 * leptons in the event and the MET vector. The variable is defined as:
 * \f[
 *    D_\zeta = p_\zeta^\text{miss} - 0.85 p_\zeta^\text{vis}
 *   \qquad
 *   p_\zeta^\text{miss} = \vec{p}_\text{T}^\text{miss} \cdot \hat{\zeta}
 *   \qquad
 *   p_\zeta^\text{vis} = (\vec{p}_\text{T}^{p_1} + \vec{p}_\text{T}^{p_2})
 * \cdot
 *   \hat{\zeta}
 * \f]
 ...
 *
 * For more information check: D. Jang, “Search for MSSM Higgs decaying to tau
 * pairs in pp collision at √s=1.96 TeV at CDF”. PhD thesis, Rutgers University,
 * 2006. FERMILAB-THESIS-2006-11.
 *
 * @param df the input dataframe
 * @param outputname the name of the output column containing the PzetaMissVis
 * value
 * @param vector_1 name of the column containing the first Lorentz vector
 * @param vector_2 name of the column containing the second Lorentz vector
 * @param vector_3 name of the column containing the third Lorentz vector (MET
 * vector)
 *
 * @return a new dataframe with the new column
 */
...
/**
 * @brief This function calculates the transverse mass \f$m_T\f$ of a two
 * particle system, where both particles are massless. The transverse mass is
 * defined as:
 *
 * \f[
 ...
 * @return a new dataframe with the new column
 */
ROOT::RDF::RNode TransverseMass(ROOT::RDF::RNode df,
                                const std::string &outputname,
                                const std::string &vector_1,
                                const std::string &vector_2) {
 ...
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the total transverse
 * mass
 * @param vector_1 name of the column containing the first Lorentz vector
 * @param vector_2 name of the column containing the second Lorentz vector
 * @param vector_3 name of the column containing the third Lorentz vector
 * (usually the missing transverse energy vector)
 *
 * @return a new dataframe with the new column
 */
ROOT::RDF::RNode TransverseMass(ROOT::RDF::RNode df,
...
/**
 * @brief This function calculates collinear mass approximation. It is defined
 * through two equations, assuming, that neutrinos of \f$\tau\f$ decays fly into
 * the same direction as visible decay products:
 * \f[
 *   p(\tau_{i}) = (1 + x_{\tau_{i}^{vis}}) \cdot p(\tau_{i}^{vis}), \qquad i =
 * 1,2
 * \f]
 * where \f$ p(...)\f$ represents the Lorentz vectors of the \f$\tau\f$ leptons
 * \f$\tau_{1}\f$ and \f$\tau_{2}\f$. The fractions \f$x_{\tau_{i}^{vis}}\f$ are
 * the additional amount of neutrino contributions, relative to the visible
 * decay products. This means, the missing transverse energy vector
 * \f$\vec{p}_{T}^{miss}\f$ can be computed as follows:
 * \f[
 *   \vec{p}_{T}^{miss} = x_{\tau_{1}^{vis}} \cdot \vec{p}_{T}(\tau_{1}^{vis})
 *    + x_{\tau_{2}^{vis}} \cdot \vec{p}_{T}(\tau_{2}^{vis})
 * \f]
 * This set of equations in turn allows to determine the values
 * \f$x_{\tau_{i}^{vis}}\f$. Example for \f$i=1\f$:
 * \f[
 *   x_{\tau_{1}^{vis}} = \frac{p_{T}^{miss}}{p_{T}(\tau_{1}^{vis})} \cdot
 *    \frac{\sin(\phi_{\tau_{2}^{vis}} -
 * \phi_{miss})}{\sin(\phi_{\tau_{2}^{vis}}
 *    - \phi_{\tau_{1}^{vis}})}
 * \f]
 * The collinear mass approximation is then computed from the sum of the full
 * \f$\tau\f$ Lorentz vectors \f$p(\tau_{i})\f$.
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the approximanted
 * collinear mass
 * @param vector_1 name of the column containing the first Lorentz vector
 * @param vector_2 name of the column containing the second Lorentz vector
 * @param vector_3 name of the column containing the third Lorentz vector (MET
 ...
 * @return a new dataframe with the new column
 */
ROOT::RDF::RNode CollinearApproxMtt(ROOT::RDF::RNode df,
                                    const std::string &outputname,
                                    const std::string &vector_1,
                                    const std::string &vector_2,
                                    const std::string &vector_3) {
 ...
 *
 * @param df input dataframe
 * @param outputname name of the output column containing the FastMtt Lorentz
 * vector
 * @param pt_1 name of the column containing the \f$p_T\f$ of the first particle
 * @param pt_2 name of the column containing the \f$p_T\f$ of the second
 * particle
 * @param eta_1 name of the column containing the \f$\eta\f$ of the first
 * particle
 * @param eta_2 name of the column containing the \f$\eta\f$ of the second
 * particle
 * @param phi_1 name of the column containing the \f$\phi\f$ of the first
 * particle
 * @param phi_2 name of the column containing the \f$\phi\f$ of the second
 * particle
 ...
 * second particle
 * @param finalstate definition of the di-tau final state decay, supported are
 * "mt", "et", "tt", "em"
 *
 * @return a new dataframe with the new column
 */
 ...
 * corresponding jet via indices that are present in nanoAODs.
 *
 * @param df input dataframe
 ...
 */
ROOT::RDF::RNode JetMatching(ROOT::RDF::RNode df, const std::string &outputname,
...
/**
 * @brief This function calculates the deltaPhi between the lepton from a W
 * boson and the visible Higgs boson decay products.
 *
 * @param df input dataframe
...
ROOT::RDF::RNode deltaPhi_WH(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    return df.Define(outputname, calculate_deltaPhi,
                     {vector_1, vector_2, vector_3});
}
