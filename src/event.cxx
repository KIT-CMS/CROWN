 ...
 * 
 * This is convenient for smearings based on random variables, as for example,
 * the jet energy resolution correction. The seeds are generated by drawing
 * an unsigned 32-bit integer from a uniform distribution.
 * 
 * @param df input dataframe
 * @param outputname name of the new column containing the generated random
 * vector
 * @param seed master seed value for the random number generator that generates
 * the seeds, if not set the answer to everything is used as default `42`
 *
...
GenerateEventSeeds(
    ...
    // initialize a random number generator with the master seed
    TRandom3 rand_gen(seed);
    
    return df.Define(
        outputname,
        [rand_gen] () {
            // return a random 32-bit integer as seed
            return static_cast<unsigned int>(rand_gen.RandInt(1, 2**32 - 1));
        },
    );
}
