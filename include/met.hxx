...
/**
 * @brief Calculates the hadronic recoil and its components parallel and
 * perpendicular to a visible system's momentum. The recoil is a crucial
 * observable in analyses with invisible particles (like neutrinos).
 *
 * The hadronic recoil (\f$\vec{u}_T\f$) is defined as the vector sum of the
 * transverse momenta of all hadronic particles in the event. By momentum
 * conservation in the transverse plane, it must balance the momentum of the
 * hard-scatter system (visible leptons/photons and invisible MET).
 *
 * It is calculated as:
 * \f[
 * \vec{u}_T = -(\vec{p}_T^{\text{visible}} + \vec{p}_T^{\text{miss}})
 * \f]
 * where \f$\vec{p}_T^{\text{visible}}\f$ is the vector sum of the transverse
 * momenta of the specified input Lorentz vectors (e.g., \f$\vec{p}_{T,
 * \ell\ell}\f$ for a Z boson event). The function then projects this recoil
 * vector onto the axis defined by the direction of the visible system's
 * transverse momentum,
 * \f$\hat{q}_T =
 * \frac{\vec{p}_T^{\text{visible}}}{|\vec{p}_T^{\text{visible}}|}\f$.
 *
 * - **Parallel Component (\f$u_\parallel\f$)**: The projection of the recoil
 * onto the visible system's axis. It is a measure of the recoil's response.
 *   \f[ u_\parallel = \vec{u}_T \cdot \hat{q}_T \f]
 * - **Perpendicular Component (\f$u_\perp\f$)**: The component of the recoil
 * orthogonal to the visible system's axis. It is a measure of the recoil's
 * resolution.
 *   \f[ u_\perp = |\vec{u}_T \times \hat{q}_T| \f] (calculated with a sign).
 *
 * The function returns a vector of two doubles: `{u_parallel, u_perp}`.
 *
 * @tparam Lorentzvectors variadic template parameter pack representing Lorentz
 * vectors
 * @param df input dataframe
 * @param outputname name of the new column containing a `std::vector<double>`
 * with the parallel and perpendicular recoil components
 * @param p4_met name of the column containing the MET Lorentz vector
 * @param lorentzvectors parameter pack of column names containing Lorentz
 * vectors of visible particles (e.g., "Lepton_p4_1", "Lepton_p4_2").
 *
...
ROOT::RDF::RNode
GetHadronicRecoil(ROOT::RDF::RNode df, const std::string &outputname,
                  const std::string &p4_met, Lorentzvectors... lorentzvectors) {
...
ROOT::RDF::RNode PropagateToMET(ROOT::RDF::RNode df,
                                ...
                                const std::string &p4_met, Args... args) {
    using namespace ROOT::VecOps;
    ...
    static_assert((n_Objects > 0) && (n_Objects % 2) == 0,
                  "Provide an even, non-zero number of object columns: first "
                  "half uncorrected, second half corrected.");
        ...
        return df.Define(
            ...
            utility::PassAsVec<n_LVs, ROOT::Math::PtEtaPhiMVector>(
                [n_Objects](
                    ...
                    // We propagate the object corrections to the MET by scaling
                    // the x and y component of the MET according to the
                    // correction of the objects
                    for (auto i = 0; i < n_Objects; ++i) {
                        float corr_x =
                            LVs.at(i).Px() - LVs.at(n_Objects + i).Px();
                        float corr_y =
                            LVs.at(i).Py() - LVs.at(n_Objects + i).Py();
                        ...
                        corrected_met.SetPxPyPzE(
                            MetX, MetY, 0,
                            std::sqrt(MetX * MetX + MetY * MetY));
                        Logger::get("lorentzvector::PropagateToMET")
                            ->debug("corrected_object pt - {}",
                                    LVs.at(n_Objects + i).Pt());
                        Logger::get("lorentzvector::PropagateToMET")
                            ->debug("uncorrected_object pt - {}",
                                    LVs.at(i).Pt());
                        Logger::get("lorentzvector::PropagateToMET")
