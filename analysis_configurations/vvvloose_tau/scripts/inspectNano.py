...
class Branch:
    def __init__(self, tree, branch):
        ...
        if self.leaf.GetLen() == 0 and self.leaf.GetLeafCount() is not None:
            self.single = False
...
def writeSizeReport(fileData, trees, stream):
    ...
    stream.write(
        ...
        """.format(
                treename=treename, scriptdata=scriptdata
            )
    ...
    for treename, (survey, _) in surveys.items():
        ...
        for s in survey:
            ...
            stream.write(
                "<td class=\"img\"><img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/blue-dot.gif' width='%d' height='%d' /></td>"
                % (s["tot"] / treetotal * 200, 10)
            )
            stream.write("<td>%.1f%%</td>" % (s["tot"] / treetotal * 100.0))
            stream.write(
                "<td>%.1f%%</td>" % ((runningtotal + s["tot"]) / treetotal * 100.0)
            )
            stream.write(
                "<td>%.1f%%</td>" % ((treetotal - runningtotal) / treetotal * 100.0)
            )
            stream.write("</tr>\n")
        ...
        stream.write(
            "<td class=\"img\"><img src=\"http://gpetrucc.web.cern.ch/gpetrucc/micro/green-dot.gif\" width='%d' height='10' />"
        ...
        if treename == "Events":
            ...
            stream.write(
                "<td class=\"img\"><img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' /></td>"
                % ((filesize - treetotal) / filesize * 100, 10)
            )
            stream.write(
                "<td>%.1f%%<sup>a</sup></td>"
    ...
    if len(surveys) > 1:
        ...
        stream.write(
            "<td class=\"img\"><img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' /></td>"
            % ((filesize - runningtotal) / filesize * 100, 10)
        )
        stream.write(
            "<td>%.1f%%<sup>a</sup></td>"
...
def writeMarkdownSizeReport(fileData, trees, stream):
    ...
    for treename, survey in surveys.items():
        ...
        for s in survey:
            ...
            stream.write(
                "| <img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/blue-dot.gif' width='%d' height='%d' />"
                % (s["tot"] / treetotal * 200, 10)
            )
        ...
        stream.write(
            "| | <img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/green-dot.gif' width='%d' height='%d' />"
            % (treetotal / filesize * 100.0, 10)
        )
        ...
        if treename == "Events":
            ...
            stream.write(
                "| | <img src='http://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' />"
                % ((filesize - treetotal) / filesize * 100, 10)
            )
            stream.write(
                "| %.1f%%<sup>a</sup> | | |\n"
