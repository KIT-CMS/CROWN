 ...
 * in the trigger selection of events for the embedding method. The scale
 ...
 * @param eta_2 \f$\eta\f$ of the subleading generator particle which
 ...
 * factors
 * @param sf_name name of the embedding selection trigger scale factor
 * correction e.g. "m_sel_trg_kit_ratio"
 *
...
SelectionTrigger(ROOT::RDF::RNode df,
    ...
    auto df1 =
        df.Define(outputname,
                  [evaluator](const float &pt_1, const float &eta_1,
                              const float &pt_2, const float &eta_2) {
                      Logger::get("embedding::scalefactor::SelectionTrigger")
                          ->debug(" pt_1 {}, eta_1 {}, pt_2 {}, eta_2 {}", pt_1,
                                  eta_1, pt_2, eta_2);
                      double sf = 1.;
 ...
 * factors
 * @param sf_name name of the embedding selection trigger scale factor
 * correction e.g. "EmbID_pt_eta_bins"
 *
 * @return a new dataframe containing the new column
 */
ROOT::RDF::RNode
 ...
 * @param outputname name of the output column containing the electron scale
 * factor
...
PtCorrection_byValue(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto correction_lambda = [sf_dm0, sf_dm1, sf_dm10, sf_dm11](
    ...
    auto df2 =
        df1.Define(outputname, correction_lambda, {pt, decay_mode_column});
    return df2;
}
 ...
 * (ID) against jets (`vsJet`) for embedding samples. The scale factors are
 * loaded from a correctionlib file using a specified scale factor name and
 * variation. The variation and the scale factor itself is binned in transverse
 * momenta (\f$p_T\f$) of hadronic taus for this function. This dependence is
 * usually used in semi-leptonic channels (\f$e\tau_h\f$, \f$\mu\tau_h\f$).
 *
 ...
 * "pt" or "dm"
 * @param variation_pt20to25 name of the scale factor variation for \f$20 \leq
 * p_T <25\f$ GeV, "nom" for nominal and "up"/"down" the up/down variation
 * @param variation_pt25to30 name of the scale factor variation for \f$25 \leq
 * p_T <30\f$ GeV, "nom" for nominal and "up"/"down" the up/down variation
 ...
 * @param variation_pt40toInf name of the scale factor variation for \f$40 \leq
 * p_T < \infty \f$ GeV, "nom" for nominal and "up"/"down" the up/down variation
 *
 * @return a new dataframe containing the new column
 *
...
Id_vsJet_lt(ROOT::RDF::RNode df,
    ...
    const std::map<float, std::string> variations = {
        {20.0f, variation_pt20to25},  {25.0f, variation_pt25to30},
        {30.0f, variation_pt30to35},  {35.0f, variation_pt35to40},
        {40.0f, variation_pt40toInf}, {100000.0f, variation_pt40toInf},
    };
